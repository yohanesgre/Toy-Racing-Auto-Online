...
public class Car : MonoBehaviour {
    ...
    public void SetterThrottle(float _t)
    {
        Throttle = _t;
    }
	...
	void Update() {
		...
		if (IsPlayerControlled) {
            ...
            if (Input.acceleration.x >= -0.1f && Input.acceleration.x <= 1f && Input.acceleration.x != 0)
            {
            ...
            if ((Input.acceleration.x < 0.25f && Input.acceleration.x > 0f) || (Input.acceleration.x > -0.25f && Input.acceleration.x < 0f))
                steerInput = 0f;
